import { Component, ChangeDetectorRef, OnInit, ViewChild, EventEmitter, Output } from '@angular/core';
...
export class OportunidadesEliminadasComponent {
  ...
  ngOnInit(): void {
    ...
    this.configuracionColumnasService.obtenerColumnasAMostrar(EnumTablas.OportunidadesEliminadas).subscribe({
      next: ({ todas, mostrar }) => {
  ...
  exportPdf(table: Table) {
    ...
    this.oportunidadService.descargarReporteOportunidadesEliminadas(data, this.loginService.obtenerIdEmpresa()).subscribe({
      next: (result: Blob) => {
